    ...
    <section class="stats" ref="statsRef">
      <div class="container">
        <div class="stats-grid">
          <div class="stat-item">
            <h3 class="stat-number">{{ stats[0].display }}</h3>
            <p>注册用户</p>
          </div>
          <div class="stat-item">
            <h3 class="stat-number">{{ stats[1].display }}</h3>
            <p>精品课程</p>
          </div>
          <div class="stat-item">
            <h3 class="stat-number">{{ stats[2].display }}</h3>
            <p>专业导师</p>
          </div>
          <div class="stat-item">
            <h3 class="stat-number">{{ stats[3].display }}%</h3>
            <p>满意度</p>
          </div>
        </div>
      </div>
...
const features = [
  {
    icon: '📚',
    title: '丰富课程',
    description: '涵盖前端、后端、设计、人工智能等热门技术领域，满足不同学习需求'
  },
  {
    icon: '🎯',
    title: '智能规划',
    description: '根据个人基础和目标，AI智能推荐学习路径，高效提升技能'
  },
  {
    icon: '💡',
    title: '实战项目',
    description: '理论结合实践，通过真实项目案例，快速掌握实用技能'
  },
  {
    icon: '👥',
    title: '社区交流',
    description: '加入学习社区，与志同道合的伙伴一起学习成长'
  }
]
...
const popularCourses = [
  {
    title: 'Web前端开发全栈课程',
    description: '从HTML/CSS基础到Vue/React框架，全面掌握前端开发技能',
    level: '初级-高级',
    students: '12,580',
    gradient: 'linear-gradient(135deg,#667eea,#764ba2)',
    svg: `<svg width="100%" height="100%" viewBox="0 0 280 160" preserveAspectRatio="xMidYMid meet">
  ...
  {
    title: 'Python数据分析与AI',
    description: '学习Python编程，掌握数据分析和机器学习核心技术',
    level: '中级',
    students: '9,234',
  ...
  {
    title: 'UI/UX设计实战',
    description: '从设计原则到工具实操，打造专业级用户体验设计作品',
    level: '初级-中级',
    students: '7,891',
...
const stats = ref([
  { target: 50000, display: 0 },
  { target: 500, display: 0 },
  { target: 150, display: 0 },
  { target: 95, display: 0 }
])
...
const animateStats = () => {
  ...
  stats.value.forEach((stat, index) => {
    ...
    const inc = increment(target)
    
    const timer = setInterval(() => {
      current += inc
      if (current >= target) {
        stats.value[index].display = target.toLocaleString()
        clearInterval(timer)
      } else {
        stats.value[index].display = Math.floor(current).toLocaleString()
      }
    }, 16)
  })
}
